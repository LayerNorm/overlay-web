...
function isInternalPlanDoc(file: string) {
  return relativeToDocs(file).startsWith('plans/')
}
...
async function checkLocalHref(sourceFile: string, href: string, failures: string[]) {
  ...
  const fromInternalPlan = isInternalPlanDoc(sourceFile)
  if (!isInsideDirectory(targetFile, docsDir)) {
    // Internal plans may link to repo paths outside docs/; the target must
    // still exist, but anchors and nav-surface rules do not apply.
    if (fromInternalPlan) {
...
async function checkStalePublicTerms(failures: string[]) {
  ...
  for (const file of files) {
    if (file === openApiFile || isInternalPlanDoc(file)) continue
    const text = await readFile(file, 'utf8')
...
async function checkPublicationReadiness(failures: string[]) {
  ...
  for (const file of markdownFiles) {
    if (isInternalPlanDoc(file)) continue
    const text = await readFile(file, 'utf8')
