...
3. Confirm the release includes root `vercel.json` with `git.deploymentEnabled.main` set to `false`. Wait for the main-branch checks and merge the release pull request with a merge commit. Verify the resulting `origin/main` SHA and verify that Vercel did not create a production deployment for it. If a deployment is unexpectedly created, cancel it before it receives production aliases and treat the release gate as failed until the trigger configuration is corrected.
