...
import { sanitizeAuditMetadata, type AuditEventRecord, type AuditRepository } from './AuditRepository'
  ...
  async append(args: Omit<AuditEventRecord, 'id' | 'createdAt'> & {
    ...
    const result = await this.db.execute<AuditRow>(sql`
      INSERT INTO audit_events (
        ...
        ${args.ipAddress ?? null}, ${JSON.stringify(sanitizeAuditMetadata(args.metadata))}::jsonb,
        ${new Date(createdAt)}
      )
