...
// Flat-rate cost estimate for budget reservation. Vercel sandbox billing is
// included in the Pro plan; this is a conservative reservation that gets
// finalized to the actual cost (often $0 for Pro plan users).
const SANDBOX_FLAT_RATE_USD = 0.10
...
export async function POST(request: NextRequest, context: AppApiRouteContext) {
  ...
  // Budget reservation — same structure as Daytona but with a flat-rate cost.
  ...
  try {
    ...
    sandbox = await runtime.create({
      ...
      persistent: false,
      hardTimeoutMs: maxDuration * 1_000,
      idleTimeoutMs: 0,
    ...
    // Finalize budget — release the reservation with actual duration.
    const durationSeconds = meteringStartedAt && meteringEndedAt
      ? Math.ceil((meteringEndedAt - meteringStartedAt) / 1000)
      : 0
    const actualCostUsd = durationSeconds > 0 ? SANDBOX_FLAT_RATE_USD * (durationSeconds / 60) : 0
    ...
    if (sandboxBudgetReservationId) {
      try {
        ...
        await generationUsagePolicy.release({
          userId,
          reservationId: sandboxBudgetReservationId,
          reason: 'sandbox_finalize_failed',
        }).catch((_error) => undefined)
      }
    }

    // Clean up the sandbox — Vercel sandboxes are ephemeral.
    if (sandbox) {
      await sandbox.stop?.().catch((_error) => undefined)
    }
    concurrencySlot?.release()
  }
}
...
> {
  ...
  const sandboxReservation = await reserveProviderBudgetFn({
    ...
    providerCostUsd: SANDBOX_FLAT_RATE_USD,
    kind: 'sandbox',
    modelId: 'vercel/sandbox',
