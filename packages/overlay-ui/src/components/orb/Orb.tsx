 ...
 * WebGL, no new dependencies. Shared across the web app and the chat/modules
 * packages so every surface renders the same mark at any size.
 *
 * Styles live in `../../styles.css` (`ovorb-*` classes).
 */
...
export type OrbProps = {
  ...
  glow?: boolean
  /**
   * Accessible label. Pass an empty string for purely decorative orbs —
   * the mark renders `aria-hidden` instead of a role/label.
   */
  label?: string
  /** Extra classes on the root span (e.g. stream-marker pulse classes). */
  className?: string
...
export function Orb({
  ...
  const dimmed = state === 'offline' ? 'ovorb-dim' : ''
  const decorative = label === ''

  return (
    <span
      role={decorative ? undefined : 'img'}
      aria-label={decorative ? undefined : (label ?? (variant === 'metal' ? 'Overlay' : 'Agent orb'))}
      aria-hidden={decorative || undefined}
      className={cn('ovorb-root', motion, dimmed, className)}
      style={{
        width: size,
        height: size,
    ...
    >
      ...
      <span className="ovorb-sheen" aria-hidden="true" />
    </span>
  )
}
...
export function OverlayMark({
  ...
  return <Orb variant="metal" size={size} state="idle" animated={false} label={label} className={className} />
}
