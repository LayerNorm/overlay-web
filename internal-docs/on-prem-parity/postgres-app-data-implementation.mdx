...
| P2 chat | Current branch | Added transactional conversation events, cursor-based browser synchronization, batched text-delta persistence, disconnect-safe background draining, stop/finalize race protection, repository-backed message/share routes, and provider-neutral public shares. |
...
| Skills | Supported | Global and project-scoped CRUD, enabled-state loading, instruction versioning, ownership, and deletion use the selected app-data provider. |
| MCP servers | Supported | Global/project configuration, encrypted credentials, tool discovery, execution policy, and hashed execution audits use the selected app-data provider. |

Unsupported or disabled:

| Surface | Why |
| --- | --- |
| Integrations/connectors | Connector account state and external integration execution are deferred. Skills and MCP remain available independently of connector providers. |
...
- A refresh or second tab reloads the aggregate persisted message from authoritative application state.
...
- Memory, tools, webhooks, and extension planning remain disabled in Postgres mode. Durable text stream recovery is supported without an external relay.
...
- Sidebar exposes Skills and MCP servers when their capabilities are enabled, while deferred connector-provider surfaces remain hidden.
- Settings loads.
...
- Connector account state and hosted/self-hosted integration execution providers.
...
### If connector controls appear in Postgres mode

Check:

- `/api/v1/capabilities` returns `capabilities.integrations=false`.
- `appDataCapabilities.supportsIntegrations=false`.
- `/api/v1/integrations` returns `501 app_data_route_not_supported` before the
  integration domain service runs.
