...
```dotenv
OVERLAY_FEATURE_WORKSPACE_WALLETS=1
OVERLAY_WORKSPACE_BILLING_ROLLOUT_STAGE=internal # off | internal | selected | general
OVERLAY_WORKSPACE_BILLING_INTERNAL_WORKSPACE_IDS=workspace_internal
OVERLAY_WORKSPACE_BILLING_SELECTED_WORKSPACE_IDS=workspace_design_partner
```
...
Convex and PostgreSQL run the same workspace billing contract:

```bash
npm run test:workspace-billing-contract:postgres
npm run test:workspace-billing-contract:convex
```
