...
---
Overlay self-hosting uses the split-license policy described in [Licensing](/legal/licensing):

- Core product code is `AGPL-3.0-or-later`.
- Reusable SDKs, contracts, protocol packages, and shared UI packages are `Apache-2.0`.
- Overlay trademarks are governed separately by the root repository `TRADEMARKS.md`.
