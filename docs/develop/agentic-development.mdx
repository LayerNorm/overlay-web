...
The staging project is explicitly branch-gated to `staging` to prevent PR and other branch builds from consuming Vercel build minutes. Its Ignored Build Step runs `scripts/vercel-ignore-build.sh` with `DEPLOY_BRANCH=staging` in both Vercel environments. The guard fails closed: only an exact `staging` Git ref runs a build; missing refs (including manual or hook-triggered deploys), unset targets, and every other branch are skipped. Preview deployment sources stay disabled. Verify these project settings when rotating Vercel projects or credentials.
