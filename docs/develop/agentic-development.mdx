...
**Current mode: direct pushes to `main`.** The owner has authorized direct pushes to `main` as the standard workflow. Work happens on `main` in the canonical checkout; commits are pushed directly without pull-request ceremony, and the staging/Vercel QA lane is skipped unless the owner requests it for a specific change. The Builder/Integration model below remains the reference for any change the owner flags for staged QA, and becomes the default again if this mode is revoked.

`main` and `staging` do not need to stay in sync — either branch may be ahead of the other and that is expected. Do not realign, mirror, or host-commit one to the other as bookkeeping; restore parity only when the owner explicitly asks. Synchronization inside the staging→main promotion flow itself (when that lane is chosen) is release mechanics, not parity maintenance.

Direct-to-main does not relax the safety rules; they become local-verification requirements:
...
Overlay uses two agent roles. A Builder works in one isolated worktree and normally submits a pull request to `staging`. The Integration agent reviews and merges the owner-selected candidate base, tests the required revision, and promotes or deploys it according to the explicit release choice. The canonical checkout stays clean and remains the trusted source for production operations.
...
When assigning work, tag [`BUILDER_AGENT_PROMPT.md`](https://github.com/LayerNorm/overlay-web/blob/main/docs/develop/BUILDER_AGENT_PROMPT.md) for implementation and PR submission, or [`INTEGRATION_AGENT_PROMPT.md`](https://github.com/LayerNorm/overlay-web/blob/main/docs/develop/INTEGRATION_AGENT_PROMPT.md) for review, staging QA, promotion, and production verification. These prompts restate the role boundaries below so an agent can execute the workflow without relying on a shortened task description.
...
Before Integration acts, it asks the owner: “Should this use a pull request or the owner-only direct-push fast path?”, “Should it target `staging` or `main`?”, and “Should Vercel deploy after integration; if so, staging or production?” If the current request already answers a question, the agent records that choice instead of asking again. In direct-push mode on `main` under `DevelopedByDev`, the default is a direct push to `main`; otherwise the default remains a pull request to `staging` for hosted QA. Production Vercel deployment always requires explicit authorization.

## Owner-only direct pushes

Repository rules require pull requests and status checks on `main` and `staging` for every actor except the specific `DevelopedByDev` user. In the current direct-push mode (see the header), when the session operates under that identity and the working branch is already `main`, completed work pushes directly to `main` — do not route it through `staging` first unless the owner explicitly asks for staging first in that task. Outside that mode this exception is a release-velocity option for the owner, not the default agent workflow, and an agent may use it only when the owner explicitly requests a direct push for the current task; full access, account identity, or an earlier instruction is not authorization.

Two stacked rulesets keep the exception narrow:
...
10. Realign `staging` to the accepted `origin/main` merge commit only when the owner requests parity or the next Builder candidate needs a clean base; use a non-rewriting fast-forward. The branches are otherwise allowed to diverge — drift between them is expected, not a defect.
