...
Before Integration creates or retargets a pull request, it asks the owner: “Should this candidate target `staging` or `main`?” and “Should Vercel deploy after merge; if so, staging or production?” If the current request already answers either question, the agent records that choice instead of asking again. The default remains `staging` for hosted QA and no Vercel deployment unless explicitly authorized. A direct-to-`main` choice still uses a protected GitHub pull request and required checks; it does not permit a direct branch push.
