...
1. **Data Sync API** (`POST /api/v1/data/sync` — infinite paginated `{status, truncates, values, syncId, pagination}` stream carrying snapshot rows then deltas incl. tombstones and bulk-replace `truncates`) — public HTTP API present in the self-hosted binary; auth = admin key. Powers the official **Fivetran source connector** (`crates/fivetran_source`) → Snowflake, BigQuery, Postgres, S3/Parquet. Airbyte covers streaming *import*. Air-gapped customers: our mirror sidecar (below) or an equivalent poller. Caveat verified 2026-09-19: the older `list_snapshot`/`document_deltas` pair does NOT emit tombstones for bulk table replaces — prefer `/api/v1/data/sync`.
...
4. **Mirror sidecar:** ✅ **Done (2026-09-19).** `examples/customer-deployment/mirror/` — a Node sidecar (`docker-compose.mirror.yml` overlay) that consumes `POST /api/v1/data/sync` (the supported Data Sync endpoint) and materializes every Convex table as `mirror.<table>(id, creation_time, ts, doc jsonb, synced_at)` in customer Postgres, component tables prefixed (`stripe_*`). Durable opaque cursor in `mirror._meta` (restart-resume), per-page transactions (idempotent replay), `truncates` → `TRUNCATE` so bulk table replacements can't drift it, i64 nanosecond `ts` preserved losslessly. Verified live: 115 tables materialized from a deployed stack; `--append` inserts and `--replace` truncates replicated in seconds. Unit + integration suites: `npm run test:mirror` (integration needs `MIRROR_TEST_DATABASE_URL`). Runbook: `mirror/README.md` + `docs/deploy-operate/self-hosted-convex.mdx`. Protocol note: `document_deltas` does NOT emit tombstones for bulk replaces — the v1 sync endpoint's `truncates` is required; do not build on `list_snapshot`/`document_deltas`.
5. **Big brain:** only if/when we host many tenants; re-read FSL before any managed-hosting shape.

## Open questions

- ~~Confirm the Data Sync API is ungated on the OSS binary~~ **Resolved (2026-09-19):** ungated. Two distinct endpoints exist on the OSS binary: `GET /api/data/sync` (WebSocket query-sync — the client's subscription protocol) and `POST /api/v1/data/sync` (the bulk **Data Sync** export endpoint — infinite paginated stream of `{status, truncates, values, syncId, pagination.nextCursor}`, admin-key auth, `selection: {"_other":"included"}` selects everything). The mirror uses the latter; verified end-to-end on `convex-backend:latest`.
- Fivetran's connector is hosted SaaS calling the deployment URL — confirm reachability story for locked-down enterprise networks (Airbyte self-hosted fallback). The `fivetran_source` crate in-tree suggests a supported path, but verify against the published connector.
