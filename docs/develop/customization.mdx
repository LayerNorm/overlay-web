...
The account panel contains identity, desktop handoff, and deployment-supported billing controls. Personal billing presents the named plan, renewal or payment status, percentage of included allowance remaining, extra top-up balance, and storage as separate indicators. One-time and automatic top-up controls live only in Account; public pricing links to them but never changes the preference. Legacy personal plans are labeled as grandfathered until the customer explicitly confirms a named-plan change. The primary sidebar rail links its compact plan status to this panel. Do not expose account-level API-key controls unless the deployment intentionally enables and tests the API-key service; a non-working credential control is worse than no control.
