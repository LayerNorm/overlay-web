...
`workspaceWallets` is disabled by default. While disabled, the server-side payer resolver always preserves personal billing even inside an organization workspace. When enabled, organization calls fail closed unless that workspace has an active billing account; they never fall back to a member's personal wallet. All enabled hosted-cost boundaries are migrated, but deployment enablement is still gated on workspace funding, spend-limit administration, `check:owner-funded-boundary`, and the billable-feature coverage test. The hosted Convex environment must set `OVERLAY_FEATURE_WORKSPACE_WALLETS=1` at the same rollout boundary so background file and memory indexing select the workspace wallet too.
