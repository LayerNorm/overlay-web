...
The default brand mark is the `OverlayMark` SVG orb component (`src/components/orb/Orb.tsx`), not the
`logoSrc` raster: the sidebar renders the orb whenever `brand.logoSrc` is the default
`/assets/overlay-logo.png`, and only renders the `next/image` raster when a custom brand overrides it.
