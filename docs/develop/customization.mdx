...
`workspaceWallets` is disabled by default. While disabled, the server-side payer resolver always preserves personal billing even inside an organization workspace. When enabled, organization calls routed through the resolver fail closed unless that workspace has an active billing account; they never fall back to a member's personal wallet. Existing spend call sites remain personal until explicitly migrated. Enable the flag only after workspace funding, spend-limit administration, and the intended caller migration are ready for that deployment.
