...
5. After the normal staging-to-main path, fast-forward `staging` to the accepted `origin/main` merge commit and push it only when the owner requests parity or the next Builder candidate needs a clean base. This is non-rewriting alignment; if it cannot fast-forward, stop and investigate the unexpected branch movement. The branches are otherwise allowed to diverge.
