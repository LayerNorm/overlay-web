...
2. ~~**1520-line `act` route (medium-high).**~~ **Landed** — the handler moved to `src/server/app-api/v1/conversations/act/ActTurnOrchestrationService.ts` (`executeActTurn`); `route.ts` is now a 16-line wrapper (`maxDuration` + `POST` → `executeActTurn(request, context, deps)`). `billable-feature-coverage` enforcement paths and the complexity baseline exemptions followed the code; `act/route.ts` dropped off `routeHandlersOverBudget`.
