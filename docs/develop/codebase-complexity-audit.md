...
> **Status: Batches 1–4 landed** (`CLEANUP 1`, 8× `CLEANUP 2`, 4× `CLEANUP 3`, `CLEANUP 4`) plus §2 items 1, 4 (showcase↔workspaces half), and 5 (`BOUNDARY REGISTRY`). Root reorg (§1), the `act` route extraction, the file-route Convex leaks, and remaining allowlist burn-down are still open.
