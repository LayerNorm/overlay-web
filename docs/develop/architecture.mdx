...
the BFF `agent-environments/operations/reconcile` route (Convex cron calls it) and the Postgres
`app-data.maintenance` job. The standalone sandbox route keeps the worst-case reservation model
