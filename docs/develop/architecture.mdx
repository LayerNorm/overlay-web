...
`BillingPayerResolver` separates workspace authorization from payment selection. The `workspaceWallets` feature flag defaults off and makes the resolver select the personal path. When enabled, organization calls that adopt the resolver require an active workspace account and have no personal fallback. Workspace reservations update the account-keyed balance and an optional `member` or `programmatic` fixed-period limit atomically; the original personal reservation method remains unchanged for rollback.
