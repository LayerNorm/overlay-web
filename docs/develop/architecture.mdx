...
called. Core workspace collaboration and Slack workspace install and identity
routes use the provider-neutral workspace repositories; remaining gated
collaboration and connector routes must not silently fall back to Convex.
...
Personal pricing uses one recurring Stripe unit Price and a code-owned named-plan catalog: Starter is quantity 8, Pro is quantity 24, and Max is quantity 96. Other positive quantities remain readable as grandfathered legacy plans. Named plan identifiers and labels are derived response fields; durable subscription records retain the existing amount, quantity, plan-kind, and plan-version compatibility fields. This personal catalog does not constrain workspace billing, which keeps its independently authorized variable-budget model.

New personal Checkout sessions accept a plan ID and derive the existing Price, quantity, and amount on the server; the temporary amount compatibility path recognizes only the three exact named-plan amounts. A user with an active, trialing, or past-due Stripe subscription cannot open a second subscription Checkout. Personal plan changes use a separate preview/confirm path against the existing single subscription item: upgrades use immediate `always_invoice` proration with `pending_if_incomplete` protection, and downgrades use a two-phase Stripe Subscription Schedule at the current item period end. Customer Portal sessions require a configured portal profile whose subscription-update feature is disabled and whose cancellation mode is `at_period_end`, so quantity changes stay inside this guarded application flow and paid access remains available through a scheduled cancellation's final period.

Personal subscription entitlements are payment-gated in both the Next/Postgres and hosted Convex webhook lanes. A paid-plan Checkout webhook is ignored unless Stripe reports `payment_status=paid`, and a new incomplete, unpaid, paused, or otherwise non-entitled subscription is stored as past due without a paid allowance. When an existing paid subscription enters recovery, its last confirmed allowance and top-up preferences are preserved instead of applying uncollected upgrade metadata. A later active or trialing webhook applies the provider quantity; deletion clears the recurring allowance and automatic top-up state. Durable and projected entitlement contracts carry the provider status, Stripe quantity, and additive `cancelAtPeriodEnd` flag so pricing, Account, sidebar, bootstrap, and native clients render the same state. Provider event idempotency still prevents duplicate webhook effects.
