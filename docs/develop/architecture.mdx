...
estimated charge. The standalone sandbox route denies outbound network access unless the operator
configures an explicit domain allowlist, rejects private and link-local destinations, terminates
the command when its non-loopback transmit-byte ceiling is exceeded, and rejects oversized
artifacts before provider transfer. Its reservation includes both the command ceiling and the
maximum bounded artifact transfer. The default `iad1` region and its configured unit rates must
move together. Blank pricing or buffer variables fall back to conservative defaults rather than
