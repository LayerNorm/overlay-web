...
- `src/contexts/**` holds app-wide client providers (`AuthContext`, `WorkspaceContext`); `src/hooks/**` holds app-wide client hooks (`useWorkspaceChanged`). Both may import `@/shared` only.
- `src/features/**` holds web feature containers and feature-local helpers. Cross-feature imports are banned; shared primitives belong in `@/shared`, `@/contexts`, or `@/hooks` (e.g. the workspace provider, routing helpers, and types that used to sit inside `features/workspaces`).
- `src/server/workflows/**` holds Vercel Workflow SDK durable orchestration (`automation-run`, `automation-schedule`, `personal-chat-work`, `workspace-agent-turn`). Workflow bodies bundle for the workflow runtime — keep their imports to `"use step"` functions and `@/` alias modules only.
- `convex/**` owns durable app state, domain mutations, scheduled work, and Stripe webhook handling.
- `scripts/` is split by purpose: `scripts/ci/` (checks, boundary rules, baselines consumers), `scripts/qa/` (smoke tests, rehearsal/readiness harnesses), `scripts/db/` (migrations, backfills, ops), `scripts/lib/` (shared script helpers). `scripts/dev-setup.sh` and `scripts/vercel-ignore-build.sh` stay at root — the latter is pinned by the Vercel project ignore-build command.
...
- `packages/overlay-ui/**` holds the shared presentation primitives (buttons, inputs, dialogs,
  and the list-page system: `Tile`/`TileGrid`/`TileIcon`/`TileSkeleton`/`CreateTile` for grid
  tiles, `ListRow` for list rows, and `HeaderSearch` for collapsible header search). List pages
  (Projects, Knowledge, Files, Agents, Extensions) must render their tiles, rows, and headers
  through these primitives instead of hand-rolled markup so spacing, radii, hover language, and
  dark-mode surfaces stay uniform.
...
App data lives exclusively in Convex: `src/server/app-data/capabilities.ts`
publishes the Convex capability set and `src/server/app-data/repositories.ts`
constructs the Convex repositories every domain service consumes. The bespoke
Postgres repository layer, Drizzle schema, app-data migrations, and parity
harness were removed; enterprise self-hosting runs the official
`convex-backend` binary (see `docs/develop/convex-only-self-hosting.md`).

Connected-agent persistence goes through the Convex-backed
...
request-replay records live in Convex tables.
...
packages so Convex does not become a public host API.
...
Workspace behavior stays behind the existing service/repository contracts while
...
execution; `conversationMessages.status` is a rendering projection. Convex
creates the assistant placeholder and AgentRun together, and completes,
fails, or cancels both records in one storage transaction.
...
Convex cron. Stop first cancels AgentRun, then
...
Vercel Sandbox is the default managed sandbox provider. Agent-environment leases no longer hold a
worst-case 24-hour reservation: `ManagedAgentSandboxBilling.reserve` takes only a small bootstrap
hold (the low-balance floor plus a short coverage window), and a per-lease meter debits the wallet
periodically while the sandbox runs. Each tick reads provider `instance.usage()` counters, computes
the delta against a cursor persisted on the lease (`usage.meteredUsage` + `meterVersion`), and
commits debit + cursor atomically through `meterSandboxLease` on the connected-agent repository —
retries replay as `lease_conflict` instead of double-charging. When the payer's remaining balance
drops under the floor (`OVERLAY_SANDBOX_LOW_BALANCE_CUTOFF_CENTS`, default $1) or a tick debit is
declined, the lease flips to `stopping`, pending commands and queued/running turns are cancelled
with a `usage_limited` terminal error, and the provider sandbox is deleted by the reaper pass; a
turn's settle is just the final meter tick plus bootstrap-hold release. All metering and cleanup
reconnects use `{resume:false}` — passive reads never wake an idle-stopped sandbox. Some providers
(Vercel) only publish cumulative usage counters once a session stops, so while a sandbox runs the
meter bills the elapsed window against a virtual wall-time cursor instead; teardown paths
(reap, harness reset) stop the instance, take one final meter read that reconciles real totals
against the virtual cursor, then delete — session usage is never lost. The meter also enforces
the lease's idle window: Vercel sandboxes have no native idle-stop (Daytona honors the same
`idleTimeoutMs` via `autoStopInterval`), so a running sandbox whose lease shows no activity for
`usage.idleTimeoutMs` (default 15 minutes) is provider-stopped while the lease stays `running` —
every turn-slice acquire heartbeats `usage.lastActiveAt`, so an actively working sandbox never
cools mid-turn, and the next turn's `resume:true` acquire wakes it again. A `running` lease past
`reservedUntil` (the 24-hour hard cap) is reaped on the next tick regardless of whether its
sandbox still answers. The meter driver runs from
the BFF `agent-environments/operations/reconcile` route (Convex cron calls it).
The standalone sandbox route keeps the worst-case reservation model
because it always denies outbound network access and can make a byte-exact provider-cost promise;
it rejects oversized artifacts before provider transfer, and its reservation includes the maximum
bounded artifact transfer. The default `iad1` region and its
...
Workspace Stripe ownership is account-keyed, never admin-keyed. Owners and admins may initialize the account and open checkout, top-up, verification, and portal flows, but Stripe metadata and durable subscription state bind to `billingAccountId` plus `workspaceId`; `userId` is retained only as the initiating actor. The Convex Stripe webhook preserves the personal metadata path, deduplicates event IDs, and rejects stale subscription state with `providerEventCreatedAt`.

Usage reservations have an evidence-gated recovery path. Work that expires before a provider starts is released automatically. Work that may have reached a provider is quarantined as `reconcile_required`; it must be finalized or released with a durable provider or internal-operation evidence reference. The five-minute Convex cron exposes queue depth and oldest age but never guesses an ambiguous charge.

Finalized usage preserves retail cost and actual provider cost independently. `UsageRepository.getBillingAccountOperationalReport` reports retail credits, actual cost, realized margin, cost coverage, reconcile queue depth, and the count older than the 15-minute response SLA. The workspace billing UI exposes this only to owners and admins.
...
- `scripts/qa/usage-reconciliation-audit.ts`
- `scripts/db/personal-billing-account-backfill.ts`
- `scripts/qa/stripe-personal-account-verification.ts`
...
- `src/server/observability/metrics.ts` — typed emission contract for BFF
  requests, Convex functions, model token breakdowns,
