...
- `bash scripts/vercel-ignore-build.sh` is the project's Ignored Build Step. It runs only when `VERCEL_GIT_COMMIT_REF` exactly matches `DEPLOY_BRANCH`; missing variables, missing branch refs, pull-request branches, manual deployments, and deploy hooks are skipped (Vercel exit `0`).
