...
The app-data repository contract suite runs against the shared development Convex deployment with `npm run test:app-data-contracts:convex`. For self-hosted backend validation, use the reference stack in `examples/customer-deployment/` and the runbook at `docs/deploy-operate/self-hosted-convex.mdx`. Redis-backed rate-limit checks run locally through `npm run test:p1:redis` against the `local-redis` service in `docker/docker-compose.better-auth.yml`.
...
This is a non-rewriting fast-forward: the release merge commit has the tested staging revision as a parent. If it cannot fast-forward, stop and inspect the unexpected branch movement instead of force-pushing. If Vercel unexpectedly creates a production deployment, cancel it before aliases are assigned and treat the deployment gate as failed until the trigger configuration is corrected. After the no-deployment check (and the alignment, when performed), report the accepted SHA as **merged but not deployed**, then stop unless the user explicitly authorizes production deployment.
