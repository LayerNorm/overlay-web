...
The Builder opens its pull request against `staging` and stops at submission. The Integration agent reviews the diff and required checks, then merges the Builder PR into `staging` through GitHub.
...
Prefer one release candidate at a time. If the Integration agent intentionally batches several Builder PRs, record every included PR and test the combined staging revision. Both Vercel projects have Preview disabled, so only the staging project's `staging` branch creates the hosted QA build.
...
The Integration agent then opens or updates a release pull request from `staging` to `main`, waits for the main-branch checks, and merges it with a merge commit. The release PR must identify every included Builder PR and the exact tested staging SHA.

The production Vercel project's Git deployment source is disabled. A release PR merge therefore updates `main` without creating a production deployment. Verify and report the exact `origin/main` SHA as **merged but not deployed**, then stop unless the user explicitly authorizes production deployment.
...
After production verification, align `staging` with the accepted `main` merge commit so the next Builder PR has a clean comparison:
...
This is a non-rewriting fast-forward: the release merge commit has the tested staging revision as a parent. If it cannot fast-forward, stop and inspect the unexpected branch movement instead of force-pushing.
