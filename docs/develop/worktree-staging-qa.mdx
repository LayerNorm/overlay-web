...
The production Vercel project's Git deployment source is disabled. A release PR merge therefore updates `main` without creating a production deployment. Verify the exact `origin/main` SHA, then align `staging` with the accepted merge commit so the next Builder PR has a clean comparison:
