...
Personal subscription responses derive `planId`, `planDisplayName`, and `isLegacyPlan` from the existing recurring amount or Stripe quantity. The named personal catalog maps Starter, Pro, and Max to quantities 8, 24, and 96 on the configured one-dollar recurring Price. Responses also carry the authoritative provider `status`, `stripeQuantity`, and additive `cancelAtPeriodEnd` state; web, bootstrap, and native subscription consumers retain all prior fields. These presentation and state fields do not replace the stored compatibility fields and do not apply personal tier constraints to workspace billing.

`POST /api/checkout` accepts the named personal `planId` and owns the Price-to-quantity mapping; the legacy `planAmountCents` input remains transitional and only accepts the three exact catalog amounts. `POST /api/subscription/change-plan` is the authenticated same-origin preview/confirm boundary for changing an existing personal subscription. It does not accept a client Stripe Price, quantity, customer, or subscription ID. The repository resolves the caller's subscription, the Stripe adapter verifies customer and Price ownership, and a confirmed legacy-to-named move explicitly ends the grandfathered quantity. Canceling subscriptions must be resumed through the safe Customer Portal before a plan change. Workspace checkout remains on its separate manager-authorized variable-budget contract.
