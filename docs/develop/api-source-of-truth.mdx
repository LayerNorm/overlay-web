...
- `POST /api/v1/conversations/stop` atomically transitions active AgentRuns to
  ...
  process. The legacy generating-message stop remains only as a migration
  fallback for runs created before AgentRun.
- `@overlay/api-client` exposes `conversations.currentRun()` and
  `conversations.stopResponse()` for these contracts. Both Convex and Postgres
  repositories must preserve identical strict lifecycle transitions.
