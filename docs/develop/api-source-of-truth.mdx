...
Persistent chat context also lists memories by `workspaceId`, so memories created by one member are available as shared context to other members of that workspace. Provider repositories must preserve the distinction: `workspaceId` owns the resource context, while `userId` records attribution and mutation ownership.
