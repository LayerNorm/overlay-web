...
- Personal workspaces are private capability boundaries. Human invitations,
  teams, channels, human/group DMs, workspace grants that reach another human,
  and sharing-policy mutations return HTTP `409` with
  `code: "personal_workspace_not_collaborative"`. Workspace billing separately
  requires an organization workspace. Private user-agent DMs
  remain supported, including private resource attachments that reach no other
  human. Public chat/file link sharing remains a separate personal feature.
