...
- Empty DM and channel selection prompts are vertically centered in the conversation canvas. A transient initial participant/history BFF failure is not evidence that a room is inaccessible: the room remains mounted while the authorized realtime transport and normal retries reconcile it, rather than showing a false “conversation unavailable” banner.
...
- Account-menu navigation inside the workspace switcher closes the switcher before routing; links such as Settings and Demo, plus Sign out, must never leave its portal visible above the next page.
- Sidebar **New note** uses the canonical `/api/v1/notes` service for both data providers, then navigates from the returned canonical note ID. It must not branch into the legacy Files create endpoint, which can leave the Notes surface without a matching update contract.
