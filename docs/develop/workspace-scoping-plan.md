...
- Room transcript UX: received messages are Slack-style flat rows (no bubble); sent messages keep the right-aligned personal-chat bubble. Hover actions float top-right on received rows. Thread entry shows `N replies` plus a latest-reply teaser. Rooms pin scroll to the latest message on open.
