...
- Opening a collaboration room marks participant read and clears unread workspace notifications for that conversation. `overlay:collaboration-notifications-changed` refreshes the cumulative Activity and chat badges immediately; Personal completion unread is added to the same Activity total.
...
- Agent mentions invoke against the `messageId` returned by `POST /api/v1/conversations/message`; clients must not rediscover that ID from asynchronously refreshed transcript state.
