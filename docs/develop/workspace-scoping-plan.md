...
- Opening a collaboration room marks participant read and clears unread workspace notifications for that conversation (`overlay:collaboration-read` refreshes sidebar badges).
- Room transcript UX: every message has an avatar + row hover background. Received messages are Slack-style flat rows (no bubble); sent messages keep the right-aligned **gray** personal-chat bubble (not the black room-self tone). Human body text is plain + mention chips (no markdown), matching personal chat. Hover actions float top-right. Thread entry shows `N replies` plus a latest-reply teaser. Rooms pin scroll to the latest message on open.
...
- BFF conversation-event long-poll remains mounted for both Postgres and Convex rooms as a fallback (token mint delay / transient auth failure). Convex subscription is still primary. Presence heartbeats, reactions, pins, and saved state are non-critical room enrichments and must never prevent membership plus message history from rendering.
