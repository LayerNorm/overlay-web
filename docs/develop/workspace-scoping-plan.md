...
- **Convex browser auth for rooms:** the shared dev Convex deployment must set `WORKOS_CLIENT_ID` and `DEV_WORKOS_CLIENT_ID` (JWKS is public; API keys are optional). Without them `requireAccessToken` always fails, `watchRoomMessages` returns `{ ok: false }`, and rooms never receive live inserts.
