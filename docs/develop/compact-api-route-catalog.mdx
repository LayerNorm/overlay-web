...
A one-line-per-route summary derived from a filesystem audit of every `src/app/api/**/route.ts`. It contains the same 197 routes as the full `api-route-catalog.mdx` but omits the per-route descriptions and implementation file lists.

- **Total routes:** 197
- **Non-versioned routes:** 35
- **`/api/v1` routes:** 162
