...
Accepted on July 26, 2026. Phase 1 shipped the workspace foundation, Phase 2 shipped the workspace-aware Chats information architecture, and Phase 3 shipped participant-scoped Direct Messages on July 29, 2026. The `workspaces` and `collaborativeChats` app features are enabled; Channels, named agents, universal sharing, and shared multi-tenant deployment remain disabled until their individual release gates pass.
...
The initial organization does not receive `#general` yet. That invariant is created atomically when Phase 4 enables Channels, so Phase 1 does not persist a fake room before room authorization exists.
...
`npm run test:workspaces:phase0` preserves the pre-multiplayer characterization. `npm run test:workspaces:phase1` validates the workspace lifecycle, `npm run test:workspaces:phase2` adds Chats IA, migration, isolation, and authorship gates, and `npm run test:workspaces:phase3` validates DMs, participant authorization, retries, presence, notifications, editing, deletion, and account cleanup across unit, Postgres, and live development Convex contracts. Later phases must update the characterization test in the same commit that deliberately changes a boundary.
