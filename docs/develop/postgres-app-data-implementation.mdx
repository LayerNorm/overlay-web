...
- Postgres app-data is not full Convex parity. Chat, projects, files, generated media, browser outputs, Daytona artifacts, memories, knowledge retrieval, durable scheduled automations, and signed outbound webhooks are supported; API keys, billing records, and integrations remain disabled or unsupported.
...
| Automations | Postgres definitions, schedule triggers, idempotent runs, attempt audit, retries, cancellation, and dead-letter state use the standalone scheduler/worker runtime. |
