...
- Postgres app-data v1 is a pilot-ready backend for users, auth identities, chat persistence and send, settings, onboarding, notes, file metadata/upload intents, account deletion, background cleanup, and real backend contract tests.
- Postgres app-data v1 is not full Convex parity. Realtime subscriptions, stream resume, automations, webhooks, projects, API keys, billing records, memory extraction, and vector search are disabled or unsupported in Postgres mode.
- Postgres vector search is not implemented yet. The intended future direction is `pgvector`, and deployments must verify their managed Postgres provider supports the `vector` extension before planning vector parity.
...
6. Tests should run against real backends where practical.
   Shared app-data contract tests run against Docker Postgres and can run against a real/local Convex deployment.
...
| `automations` | `ConvexAutomationRepository` | Unsupported repository |
| `billing` | `ConvexBillingRepository` | Unsupported repository |
| `usage` | Unsupported placeholder | Unsupported placeholder |
...
| --- | --- | --- |
| Chat list/read/create/update/delete | Degraded | No Convex realtime sync or stream resume. |
| Chat send | Degraded | Direct model chat path with explicit usage policy; no memory/tools/webhooks. |
| Chat title generation | Degraded | Uses configured `chatUsagePolicy`; works in billing-disabled Postgres pilot mode. |
| Settings | Supported | Backed by `user_settings`. |
| Onboarding | Supported | Backed by `onboarding_state`. |
| Notes | Supported | Backed by `notes`. |
| File metadata | Degraded | Metadata, upload intents, duplicate promotion, and recursive delete supported; vector indexing disabled. |
| Account deletion | Supported | Deletes auth user/session and app-data rows. |
| Background maintenance | Supported | Explicit script/cron command, not Convex scheduler. |
| Model catalog | Supported | Does not require app-data persistence. |
...
| Projects | Schema exists, but app behavior and UI are disabled until service parity is complete. |
| Automations | Scheduler/execution/retry model is Convex-backed. |
| Webhooks | Delivery retries and durable state are not implemented for Postgres. |
| API keys | No Postgres API key repository yet. |
| Billing records | Billing must be disabled with Postgres app-data v1. |
| Usage accounting | Chat uses `UnlimitedUsagePolicy`; no persistent Postgres usage audit backend yet. |
| Memory extraction | Background jobs and vector storage are not implemented. |
| Vector search | Planned future `pgvector` direction, not available today. |
| Generated outputs | Still Convex/storage-path dependent. |
...
- Postgres mode relies on the active HTTP stream and local runtime state during a send.
- `/api/v1/generate-title` is now allowed in Postgres mode as a degraded chat-persistence route.
- Chat title generation uses `chatUsagePolicy`, not direct Convex billing queries.
- Memory, tools, webhooks, extension planning, and stream resume are disabled in Postgres mode.
...
Supported or partially supported operations:

- File/folder metadata CRUD.
- Recursive subtree delete.
- Duplicate detection and duplicate promotion.
- Upload intent creation/finalization/expiration.
- Owner-scoped file listing.
- Share metadata fields.
- Quota accounting as an explicit no-op in billing-disabled Postgres pilot mode.

Still not full parity:

- Vector indexing is disabled.
- Knowledge chunk purge is limited to metadata boundaries because Postgres vector chunks do not exist yet.
- Generated outputs remain outside the current Postgres pilot surface.
...
Convex scheduled functions are not available in Postgres mode. The implementation adds an explicit maintenance command:
...
- Run this command from the deployment scheduler, Kubernetes CronJob, system cron, Vercel Cron, or equivalent.
- Do not assume Convex crons are running when `database.provider=postgres`.
...
- Persistent idempotency/nonce garbage collection, unless backed by a future Postgres implementation.
...
```bash
docker exec -it overlay-app-data-postgres psql -U overlay_app -d overlay_app
```
...
6. Run app-data migrations.
7. Deploy the app.
8. Smoke auth, session, chat send, chat title generation, settings, notes, files, account deletion, and sign out.
9. Configure the background maintenance scheduler.
...
- Sidebar does not show disabled surfaces like Extensions, Projects, Automations, Skills, or MCPs in Postgres pilot mode.
- Settings loads.
...
- Removing Convex browser subscriptions in Postgres mode can reduce client-side network chatter, but it also removes realtime updates and stream resume until those are rebuilt on another backend.
...
- Onboarding.
- Notes CRUD.
- File metadata and upload intent lifecycle.
- Account deletion cleanup.
- Background maintenance command.
...
6. Add Postgres-backed durable scheduler/work queue if automations, webhooks, or memory extraction become pilot requirements.
