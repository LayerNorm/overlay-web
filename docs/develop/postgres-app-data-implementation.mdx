...
- Postgres app-data is not full Convex parity. Chat realtime, durable text recovery, stop, sharing, and second-tab synchronization are supported; automations, webhooks, projects, API keys, billing records, memory extraction, and vector search remain disabled or unsupported.
