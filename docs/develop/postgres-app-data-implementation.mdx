...
- Postgres app-data is not full Convex parity. Chat, projects, files, generated media, browser outputs, Daytona artifacts, memories, and knowledge retrieval are supported; automations, webhooks, API keys, billing records, and integrations remain disabled or unsupported.
...
| Automatic memory extraction | Explicit saved-memory CRUD and retrieval are supported; automatic model-driven extraction policy remains a separate product behavior. |
