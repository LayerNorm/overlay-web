...
| `INTERNAL_API_SECRET` | Always | Shared app-to-Convex secret, matching the selected Convex deployment. |
...
| Web search and fetch | `ELO_API_KEY` (ELO routes `web_search`/`deep_search` to Tavily and Parallel) and `TINYFISH_API_KEY` (`web_fetch` page extraction). Server-side only; both resolve from WorkOS Vault first with env fallback. |
...
On the Convex deployment, configure:
...
When `auth.provider="better-auth"`, `BETTER_AUTH_DATABASE_URL` points at a
separate auth-scoped Postgres database owned by Better Auth. It is unrelated to
application data, which always lives in Convex.
...
setting them only on Vercel does not configure the Convex email worker. Keep
`OVERLAY_EMAIL_PROVIDER=none` to disable all Overlay transactional email. WorkOS
or the selected IdP remains responsible for verification and password-reset
messages.
