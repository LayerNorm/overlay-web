...
| Database | `convex`, `postgres` | `convex` is the supported app-data provider. `postgres` config, local Docker, and core schema migrations are available; app-data repository adapters are still required before the web app can boot in Postgres mode. |
