...
| Web search | `elo`, `ai-gateway`, `perplexity`, `tavily`, `none` | Search queries, result snippets, and fetched page content | `elo` and `none` are supported. `elo` routes `web_search`/`deep_search` through ELO to Tavily and Parallel and `web_fetch` to TinyFish (`ELO_API_KEY`, `TINYFISH_API_KEY`). `ai-gateway`, `perplexity`, and `tavily` are declared but rejected — the gateway search path was removed. |
