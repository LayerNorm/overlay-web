...
| Database | `convex`, `postgres` | `convex` is the supported app-data provider. `postgres` config, local Docker, and migrations are available for Phase 1 setup; app-data repository adapters are still required before the web app can boot in Postgres mode. |
