...
| Database | `convex`, `postgres` | `convex` is the hosted/default app-data provider. `postgres` supports users, chat, projects, files, notes, generated outputs, memory, knowledge retrieval, durable scheduled automations, billing controls, skills, and MCP servers. Connector account state and integration execution remain gated until the Postgres parity phase. |
...
| Secrets | `env`, `workos-vault`, `aws-secrets-manager`, `vault`, `none` | Provider credentials | `env` is the general runtime path. `workos-vault` supports hosted BYOK flows. `aws-secrets-manager` supports BYOK provider keys when app data uses Postgres; Postgres stores only the opaque secret ARN. Vault remains declared but unsupported. |
