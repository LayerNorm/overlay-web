...
Postgres app-data support is split into phases. Operators can provision, validate, migrate, and smoke-test the app-data schema now. Run with `billing.provider="none"` and keep automations, webhooks, integrations, memory extraction, vector search, and API keys disabled until their parity phases land. Projects, files, generated media, browser outputs, and Daytona artifacts are supported.
...
Postgres app-data does not use Convex crons. Run `npm run app-db:worker` as a separately supervised service. Its Postgres scheduler enqueues maintenance, coordination cleanup, model-catalog refresh, S3 reconciliation, and output-retention work; leased workers execute jobs with retry and dead-letter state. The standalone `npm run app-db:maintenance` command remains available for manual recovery. `compliance.retention.fileDays` governs generated media/browser outputs and `compliance.retention.sandboxArtifactDays` governs Daytona artifacts. Webhook delivery, automation execution, memory extraction, and vector indexing remain capability-gated until their domain phases add complete handlers.
