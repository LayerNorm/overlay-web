...
For licensing and brand constraints, see [Licensing](/legal/licensing) and [Legal Self-Hosting Notes](/legal/self-hosting-obligations). The short version: first-party material in this repository is `AGPL-3.0-only`, separate submodules and third-party material keep their own licenses, and modified distributions need their own branding unless LayerNorm Inc. gives written permission.
...
See [Managed Cloud Enterprise](./cloud-managed) for the managed-cloud option, [Self-Hosted Convex Backend](./self-hosted-convex) for running the Convex backend itself on customer infrastructure, and [Tenancy And Role Model](./tenancy) for the tenant boundary.
...
| `INTERNAL_API_SECRET` | Always | Shared server-to-Convex secret matching the selected Convex deployment. |
| `INTERNAL_SERVICE_AUTH_SECRET` | Automations | Shared by the app runtime and the selected Convex deployment when Convex executes automation callbacks. Keep it distinct from `INTERNAL_API_SECRET` and aligned per environment. |
...
| `S3_BUCKET_NAME`, `S3_REGION`, `S3_ENDPOINT_URL`, `S3_ACCESS_KEY_ID`, `S3_SECRET_ACCESS_KEY`, `S3_FORCE_PATH_STYLE` | `storage.provider="s3"` | On AWS, omit access keys to use the task or instance role. For other S3-compatible storage, configure both keys together; use `S3_ENDPOINT_URL` and `S3_FORCE_PATH_STYLE=true` when path-style access is required. |
...
| `database` | Convex app-data selection (`convex` only) and backend connection settings. |
...
| Database | `convex` | App records, messages, files metadata, usage state | Convex deployment region | `convex` is the only supported app-data provider. Enterprise self-hosting runs the official `convex-backend` binary; see [Self-hosted Convex backend](/deploy-operate/self-hosted-convex). |
...
| Vector search | `convex`, `pgvector`, `pinecone`, `none` | Embeddings and searchable text chunks | Database/vector provider region | `convex` and `none` are supported. `pgvector` was removed with the Postgres app-data provider. `pinecone` is declared but rejected until an adapter exists. |
...
| Sandboxes | `vercel`, `daytona`, `e2b`, `local-firecracker`, `none` | Commands, staged files, generated artifacts | Sandbox provider region | `vercel` is the hosted default and `daytona` remains an explicit opt-in. `e2b` and `local-firecracker` are declared but rejected until adapters exist. |
...
| Secrets | `env`, `workos-vault`, `aws-secrets-manager`, `vault`, `none` | Provider credentials | Secret manager region | `env` and WorkOS vault-backed provider keys are supported for hosted flows. BYOK provider keys use AWS Secrets Manager when `providers.secrets.provider="aws-secrets-manager"`; Convex stores only the opaque secret ARN. Vault remains declared but unsupported. |
...
Store it as `MCP_CREDENTIAL_ENCRYPTION_KEY` on the web runtime. During
...
### Convex app-data backend

Convex is the only app-data provider. For enterprise self-hosting, run the
official `convex-backend` binary with the reference stack in
`examples/customer-deployment/` and the operations runbook at
[Self-hosted Convex backend](/deploy-operate/self-hosted-convex). Scheduled
automations, memory extraction, knowledge indexing, retention, and signed
outbound webhooks all execute inside the Convex deployment — no separate worker
process is required. Automations additionally require
`INTERNAL_SERVICE_AUTH_SECRET` on both the app and the Convex deployment.

When changing `OVERLAY_EMBEDDING_MODEL_VERSION`, Convex knowledge maintenance
crons reindex stored content automatically — let them drain before retiring the
old embedding model configuration.

For local setup, point `NEXT_PUBLIC_CONVEX_URL` at a `convex dev` session
(`npx convex dev`) or a local self-hosted backend from
`examples/customer-deployment/`.

For a managed or self-hosted cloud backend, deploy the `convex/` tree once per
environment with `convex deploy` and set `NEXT_PUBLIC_CONVEX_URL` plus the
matching `INTERNAL_API_SECRET` / `INTERNAL_SERVICE_AUTH_SECRET` on the app.

`BETTER_AUTH_DATABASE_URL` is only for Better Auth's own tables — never for
application data, which lives exclusively in Convex.

### Billing, API keys, and administrators

Stripe subscription records, exactly-once top-up grants, and durable Stripe
event deduplication are handled inside Convex through the
`@convex-dev/stripe` component. Register the deployment's Convex Stripe
webhook endpoint (printed by the component setup) as the single Stripe
webhook; there is no Next.js Stripe webhook route.

API key values are returned only on create or rotation. Convex stores an
...
Bootstrap the first administrator from a trusted shell after the user has
signed in once and exists in the Convex `users` table:

```bash
convex run admin/authorization:grantRole \
  '{"userId":"USER_ID","role":"admin","reason":"initial operator"}'
```
...
Background work runs as Convex crons and scheduled functions inside the
deployment — maintenance, coordination cleanup, model-catalog refresh, S3
reconciliation, output retention, knowledge maintenance, due automation scans,
and Daytona reconciliation. Automation occurrences are idempotent, each attempt
is audited, overlapping runs obey the configured `skip` or `queue` policy, and
cancellation is checked before execution. Outbound webhook events are
deduplicated per subscription/event, signed over `timestamp.payload` with
HMAC-SHA256, and retain delivery-attempt and dead-letter records. Signing
secrets are returned only when a subscription is created or rotated; dead-letter
redrive creates a new immutable delivery record. Daytona reconciliation adopts
only correctly labeled workspaces, rejects unowned provider records, marks
missing workspaces, and advances active metering windows idempotently.
`compliance.retention.fileDays` governs generated media/browser outputs and
`compliance.retention.sandboxArtifactDays` governs Daytona artifacts.

Chat realtime uses Convex subscriptions end to end — clients subscribe through
`ConvexReactClient` and receive mutations as they commit. It does not require
sticky load-balancer sessions, long-poll endpoints, or a separate events
table; reconnects resync from the Convex query cache automatically.

Chat mode streams directly from the web runtime and persists one final assistant-message snapshot. After a browser disconnect, clients use the authoritative AgentRun status and reload the final response when it appears; exact SSE-frame replay is intentionally not part of the persistence contract.

The migration shape was:

1. Consolidate repository/context boundaries before adding new persistence.
2. Add capability gates and route classification for every app surface.
3. Implement chat with an explicit usage policy.
4. Move business invariants into domain services.
5. Replace the bespoke Postgres app-data path with real Convex contract tests.
...
| `webSearch` | Web search/fetch tool exposure (`web_search`, `deep_search`, `web_fetch`). |
...
Use this path when an enterprise operator wants a source checkout with a plain Compose install. The default Compose file pulls a prebuilt `ghcr.io/layernorm/overlay-web` image; local source builds are opt-in through `docker/docker-compose.build.yml`.
...
`docker/docker-compose.yml` mounts `overlay.config.json` into the container. The compose file intentionally does not run object storage, auth, database, browser automation, or sandbox services; those are enterprise-owned providers selected through config.
...
Existing `docker/docker-compose.onprem.yml` and `.env.onprem.example` files are retained as compatibility aliases for older operator instructions.
