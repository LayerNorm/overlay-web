...
| `INTERNAL_SERVICE_AUTH_SECRET` | Automations | Shared by the app runtime and the selected Convex deployment when Convex executes automation callbacks. Keep it distinct from `INTERNAL_API_SECRET` and aligned per environment. Postgres-only deployments keep it on the app and worker runtimes. |
...
| Secrets | `env`, `workos-vault`, `aws-secrets-manager`, `vault`, `none` | Provider credentials | Secret manager region | `env` and WorkOS vault-backed provider keys are supported for hosted flows. In Postgres mode, BYOK provider keys use AWS Secrets Manager when `providers.secrets.provider="aws-secrets-manager"`; app-data stores only the opaque secret ARN. Vault remains declared but unsupported. |
