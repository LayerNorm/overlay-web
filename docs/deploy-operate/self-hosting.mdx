...
For tenant boundaries and enterprise role modeling, see [Tenancy And Role Model](tenancy). The short version: self-hosted and managed-cloud enterprise deployments are single-customer deployments; students, teachers, parents, admins, departments, and classes are roles or groups inside that deployment, not tenants.
...
The manual UI checklist is in [Phase 6 Release Gates](release-gates).
