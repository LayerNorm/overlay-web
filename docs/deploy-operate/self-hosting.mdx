...
Chat streams directly from the web runtime. For persistent conversations, the server continues consuming the model stream after a browser disconnect, persists batched text deltas, and lets reconnecting clients reload the authoritative message snapshot. No external stream relay is required or supported.
