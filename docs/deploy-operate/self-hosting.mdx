...
| Database | `convex`, `postgres` | App records, messages, files metadata, usage state | Convex deployment region today, or customer Postgres region in Postgres mode | `convex` supported. `postgres` supports the pilot app-data slice for auth-linked users, chat, settings/onboarding, notes, files metadata/upload intents, account deletion, and explicit maintenance jobs. Automations, webhooks, memory extraction, vector search, API keys, and projects remain disabled in Postgres mode. |
...
| Vector search | `convex`, `pgvector`, `pinecone`, `none` | Embeddings and searchable text chunks | Database/vector provider region | `convex` and `none` supported. `pgvector` is the planned Postgres-native direction, but no adapter exists yet. `pinecone` is declared but rejected until an adapter exists. |
...
Postgres app-data support is split into phases. Operators can provision, validate, migrate, and smoke-test the pilot app-data schema now. For pilot v1, run with `billing.provider="none"` and keep automations, webhooks, memory extraction, vector search, API keys, and projects disabled.

Postgres app-data v1 does not include vector parity. Future Postgres-native memory and knowledge search should use `pgvector`; do not keep Convex enabled in the browser as a hidden vector sidecar for Postgres mode. Until that adapter exists, set:
...
OVERLAY_DATABASE_URL="postgres://..." npm run app-db:maintenance
```
...
Postgres app-data does not use Convex crons. Schedule `npm run app-db:maintenance` from the deployment scheduler, Kubernetes CronJob, system cron, or equivalent. The command handles stale generating-message cleanup, inactive/old message delta cleanup, and old empty conversation cleanup. Webhook delivery retries, automation scheduling, memory extraction jobs, and vector indexing are capability-gated off in Postgres pilot v1.
