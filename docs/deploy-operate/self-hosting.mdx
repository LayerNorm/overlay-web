...
For the simplest operator path, use the one-command installer:
...
Use this path when developing or validating source changes. Production self-hosting should prefer the prebuilt-image installer in [One-Command Install](/deploy-operate/one-command-install).
