...
| EC2 | Ubuntu 24.04, at least 40 GB of encrypted gp3 storage, and `t3.xlarge` or another 4-vCPU instance for a reasonably fast source build. A `t3.large` works but can take about 17 minutes to build. |
...
<Steps>
  ...
  <Step title="Validate and build the release image">
    ...
    all use that same image. Building the `web` target avoids asking Compose to
    render the same shared image three times.
  </Step>
