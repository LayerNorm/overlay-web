...
- A Convex deployment for app data: Convex Cloud, or the official
  `convex-backend` binary for self-hosted deployments (see
...
For the current end-to-end Convex backend and Better Auth procedure, use
...
- `docker/docker-compose.yml` for the default image-based runtime.
- `docker/docker-compose.build.yml` for local source builds.
...
| App starts but cannot load data | Check `/api/v1/capabilities`, then verify `NEXT_PUBLIC_CONVEX_URL`, the matching `INTERNAL_API_SECRET`, and that the Convex deployment is reachable and current. |
