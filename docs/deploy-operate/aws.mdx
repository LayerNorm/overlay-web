...
| Database | Encrypted Multi-AZ RDS PostgreSQL or Aurora PostgreSQL writer; separate app/auth databases and roles; install `pgvector` before app-data migrations; automated backups and restore test. |
...
```bash
NODE_OPTIONS=--conditions=react-server node_modules/.bin/tsx scripts/db/app-data-migrate.ts && \
