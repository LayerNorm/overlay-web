...
- Matching Convex environment variables for Better Auth JWT verification, since app data always lives in Convex.

Keep the Better Auth Postgres database auth-scoped. It stores Better Auth users, sessions, and linked SSO accounts; it does not replace Convex app data. `BETTER_AUTH_DATABASE_URL` is used only for Better Auth state — application records always live in the Convex deployment.
