...
- Merge PRs once required checks are green. Do not wait for a second human reviewer. Sole merge authority on `LayerNorm/overlay-web` is `DevelopedByDev`; coding agents acting on that GitHub CLI or MCP identity may squash-merge with `gh pr merge <n> --squash --delete-branch`. Do not re-enable required approving reviews on `main` — a sole author cannot approve their own PRs.
...
- `main` on `LayerNorm/overlay-web` requires a PR, squash-only linear history, conversation resolution, and the required CI/security checks. It does **not** require an approving review. Push and merge are restricted to `DevelopedByDev` (agents using that GitHub CLI/MCP identity count as the same user). Repository admins have a pull-request-only ruleset bypass for stuck checks; force-push and deleting `main` remain blocked.
