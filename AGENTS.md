...
- The Integration agent owns review, dependency ordering, conflict resolution, merge into `staging`, staging deployment and QA, the `staging` to `main` release PR, merge method, production verification, and the non-rewriting fast-forward that realigns `staging` with the accepted `main`. Do not require separate Reviewer or Release Authority agents.
