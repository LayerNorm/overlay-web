...
- Production Vercel deployments are manual. The production project's Git deployment source is disabled, so merging or pushing `main` must not be described as a deployment. Only start an explicit production web deployment after the user authorizes that release; otherwise stop after the `main` merge and report the exact SHA as merged but not deployed.
