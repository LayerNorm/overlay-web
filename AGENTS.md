...
- Production Vercel deployments are manual. Root `vercel.json` sets `git.deploymentEnabled.main` to `false`; keep that rule in every release so merging or pushing `main` does not create a production deployment. After the release merge, verify that Vercel did not create a production deployment, realign `staging` with `main`, then stop and report the exact SHA as merged but not deployed unless the user authorizes production deployment.
- Use only two development roles unless the user explicitly requests more. The Builder implements and validates one change, updates documentation and `CHANGELOG.md`, opens a PR against `staging`, and stops. The Integration agent reviews and merges the PR into `staging`, tests the exact staging revision, then promotes `staging` to `main` through a release PR and verifies production.
...
- Public npm packages use `@layernorm/<product>-<component>`: `@layernorm` is the organization scope, `overlay` is the product, and the remaining suffix identifies the component. For example, publish the Agent Host as `@layernorm/overlay-agent-host` and its bridge contract as `@layernorm/overlay-agent-bridge-protocol`. Apply this pattern to every new public package; workspace-only packages may continue using the internal `@overlay/*` names.
...
- `main` on `LayerNorm/overlay-web` requires a PR, conversation resolution, and the required CI/security checks. Merge commits preserve meaningful reviewed commit series; squash remains available for trivial or noisy branches, and rebase merging is disabled. It does **not** require an approving review. Push and merge are restricted to `DevelopedByDev` (agents using that GitHub CLI/MCP identity count as the same user). Repository admins have a pull-request-only ruleset bypass for stuck checks; force-push and deleting `main` remain blocked.
...
- A Builder owns one feature worktree and opens a focused pull request against `staging`. The Builder stops at PR submission and does not merge, deploy staging, or promote production.
- The Integration agent reuses one dedicated `staging` worktree for all sequential PRs; it does not create a new integration worktree per PR or check out feature branches there. It owns review, dependency ordering, conflict resolution, merge into `staging`, staging deployment and QA, the `staging` to `main` release PR, merge method, production verification, and the non-rewriting fast-forward that realigns `staging` with the accepted `main`. A temporary worktree is reserved for exceptional conflict or side-by-side investigation and must be removed afterward. Do not require separate Reviewer or Release Authority agents.
- Merging the release PR does not deploy production. After the merge, verify the exact `origin/main` SHA and fast-forward `staging` to it without rewriting history. Then stop in a merged-but-not-deployed state unless the user explicitly authorizes deployment. When authorized, deploy that exact revision from the clean canonical `main` worktree, verify the production web deployment, deploy production Convex only when applicable and only after the web deployment is live, then run production smoke tests.
...
- Hosted pre-production QA runs only through the `staging` branch and its dedicated Vercel project. The staging project uses `bash scripts/vercel-ignore-build.sh` with `DEPLOY_BRANCH=staging` in Preview and Production; for that target the guard fails closed and skips every non-`staging` ref, missing ref, manual deploy, and hook. The shared guard preserves no-ref manual releases for any separate, explicitly configured target. Preview deployment sources remain disabled, so feature branches must not create hosted builds. Follow `docs/develop/worktree-staging-qa.mdx` for QA and promotion only.
...
| `docs/develop/api-route-catalog.mdx` | Any change to a web API route's existence, methods, behavior, authentication boundary, or implementation files. Keep the exhaustive route descriptions current. |
